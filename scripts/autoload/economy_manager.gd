...
func add(currency_id: String, amount: int, reason: String = "") -> int:
	...
	if currency_id == "coins":
		...
		GameManager.state.lifetime_stats.coins_earned = int(GameManager.state.lifetime_stats.get("coins_earned", 0)) + amount
...
func perform_prestige() -> int:
	...
	var prestige_tree: Dictionary = GameManager.state.get("prestige_tree", {}).duplicate(true)
	var achievements: Dictionary = GameManager.state.get("achievements", {}).duplicate(true)
	var daily_quests: Dictionary = GameManager.state.get("daily_quests", {}).duplicate(true)
	var tutorial: Dictionary = GameManager.state.get("tutorial", {}).duplicate(true)
	var lifetime_stats: Dictionary = GameManager.state.get("lifetime_stats", {}).duplicate(true)
	...
	GameManager.state.prestige_tree = prestige_tree
	GameManager.state.achievements = achievements
	GameManager.state.daily_quests = daily_quests
	GameManager.state.tutorial = tutorial
	GameManager.state.lifetime_stats = lifetime_stats
...
func prestige_node_cost(id: String) -> int:
	var item := DataManager.get_prestige_node(id)
	var level := GameManager.prestige_node_level(id)
	var growth := float(DataManager.balance.get("prestige", {}).get("node_cost_growth", 1.7))
	return int(ceil(float(item.get("base_cost", 1)) * pow(growth, level)))

func purchase_prestige_node(id: String) -> bool:
	var item := DataManager.get_prestige_node(id)
	if item.is_empty():
		return false
	var level := GameManager.prestige_node_level(id)
	if level >= int(item.get("max", 1)):
		EventBus.notification_requested.emit("Legacy node is already maxed", false)
		return false
	if not spend("prestige_tokens", prestige_node_cost(id), "legacy node " + id):
		return false
	GameManager.state.prestige_tree[id] = level + 1
	EventBus.state_changed.emit("prestige_tree")
	EventBus.notification_requested.emit("Upgraded " + str(item.name), true)
	SaveManager.queue_save()
	return true

func calculate_meal_payment(recipe: Dictionary, spend_multiplier: float, tip_multiplier: float, quantity: int = 1) -> Dictionary:
	...
	var gross := float(recipe.get("price", 0)) * quantity * spend_multiplier * prestige_multiplier * (1.0 + GameManager.bonus("meal_value")) * (1.0 + GameManager.bonus("table_capacity")) * (1.0 + GameManager.blessing("income"))
	var net: int = max(1, int(round(gross - float(recipe.get("cost", 0)) * quantity)))
	var tip := 0
	...
	return {"coins":net + tip,"tip":tip,"xp":int(recipe.get("xp", 0)) * quantity}
